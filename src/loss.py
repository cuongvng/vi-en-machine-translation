...
class MaskedPaddingCrossEntropyLoss(nn.CrossEntropyLoss):
    def forward(self, pred, label, valid_len):
        ...
        """
        self.reduction = 'none'
        masks = self._mask_padding_tokens(torch.ones_like(label), valid_len)
    ...
    def _mask_padding_tokens(self, X, valid_len, value=0):
        """
        ...
        ```
        """
        masks = torch.arange(X.shape[1]).unsqueeze(dim=0) # (1, seq_len)
